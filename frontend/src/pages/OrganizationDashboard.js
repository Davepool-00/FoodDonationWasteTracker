...
const OrganizationDashboard = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/api/received-donations/')
      .then(response => setReceivedDonations(response.data))
      .catch(error => console.log('Error fetching received donations:', error));

    // Fetch pending donations
    axios.get('http://127.0.0.1:8000/api/pending-donations/')
      .then(response => setPendingDonations(response.data))
      .catch(error => console.log('Error fetching pending donations:', error));
  }, []);
